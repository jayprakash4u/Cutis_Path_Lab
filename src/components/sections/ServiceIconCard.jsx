...
export default function ServiceIconCard({
  ...
  return (
    <article
      className={`flex h-full flex-col items-center rounded-xl border border-slate-200 border-t-4 border-t-brand-500 bg-white p-5 shadow-sm transition-all duration-300 group hover:border-brand-500 hover:shadow-lg sm:p-6 md:p-7 ${className}`}
    >
      <div className="mb-4">
        <IconComponent size={52} className="h-12 w-12 sm:h-14 sm:w-14" />
      </div>
      <h3 className="mb-2 text-center text-sm font-semibold text-slate-800 transition-colors group-hover:text-brand-600 sm:text-base">
        {service.name}
      </h3>
