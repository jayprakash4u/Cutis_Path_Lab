...
/* Artwork stays in code and is picked by the row's `iconKey`; the copy is
   edited in Admin → Home page → Why choose us. */
const ICONS = {
  team: (
...
const ICON_ORDER = ["team", "report", "support", "quality"];
...
const DEFAULT_STEPS = [
  {
    badge: "01",
    title: "Expert Team",
    description: "Our team consists of highly skilled & experienced pathologists",
    iconKey: "team",
  },
  {
    badge: "02",
    title: "Accurate Reports",
    description: "We provide accurate and reliable test reports",
    iconKey: "report",
  },
  {
    badge: "03",
    title: "24/7 Support",
    description: "We are available 24/7 for your support",
    iconKey: "support",
  },
  {
    badge: "04",
    title: "Quality Assurance",
    description: "International quality standards with NABL accreditation",
    iconKey: "quality",
  },
];

export default function Stats({ section, items }) {
  const steps = items?.length ? items : DEFAULT_STEPS;

  return (
    <Section tone="white">
      <SectionHeading
        title={section?.title || "Why patients choose Cutis Path Lab"}
        subtitle={section?.subtitle || "Accredited processes, experienced people, and results you can act on."}
      />
            ...
            {steps.map((s, i) => (
              ...
              >
                ...
                <div className="hidden shrink-0 sm:block">
                  {ICONS[s.iconKey] || ICONS[ICON_ORDER[i % ICON_ORDER.length]]}
                </div>
              </div>
            ))}
          </div>
